...
public class MainActivity extends GoogleApiActivity {
	...
	protected void onStart() {
		...
		AsyncTask.execute(new Runnable() {
				@Override
				public void run() {
					LocationRequestHandler test = new LocationRequestHandler(MainActivity.this);
					test.handle(new LocationRequest("123", "test@example.com", DateTime.now(UTC), Duration.millis(10000)));
				}
		});
